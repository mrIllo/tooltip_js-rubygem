...
desc 'Update tooltip.js assets to the latest npm release'
task :update do # rubocop:disable Metrics/BlockLength
  require 'net/http'
  require 'uri'
  uri = URI.parse('https://unpkg.com/tooltip.js')
  fetch = lambda do
    ...
    when Net::HTTPRedirection
      ...
      uri.path = URI.parse(response['location']).path.gsub(/\/\.\//, '/')
      fetch.call
    else
      response.error!
    end
  ...
  File.write(File.join('assets/javascripts/tooltip.js'), src)

  version_path = File.join('lib/tooltip_js/version.rb')
